...
| Item | Reason | Risk level | Files likely affected | Verification command | Presentation value |
| --- | --- | --- | --- | --- | --- |
| Add transactional outbox, broader idempotency coverage, and retry or DLQ handling for messaging | S29 covers selected critical endpoints, but the RabbitMQ flow is still lightweight and does not yet provide outbox-grade durability. | Medium | `backend/enrollment-service/*`; `backend/billing-service/*`; `backend/notification/*`; `db/*` | `rg -n "outbox|idempot|DLQ|retry" backend db` | Medium: valuable for roadmap discussion after the current delivery, not required now. |
