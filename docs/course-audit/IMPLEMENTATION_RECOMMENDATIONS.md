...
| Maintain the implemented Prometheus alert package and explain its limits clearly | Prometheus alert rules now exist; the remaining value is keeping the presentation honest about what is active versus what still needs Alertmanager and dashboards. | Low | `infra/prometheus/*`; `docs/course-audit/*`; `docs/demo/*` | `Start-Process "http://localhost:9090/alerts"`; `Start-Process "http://localhost:9090/rules"` | High: shows observability maturity with live evidence instead of only a proposal. |
...
| Item | Reason | Risk level | Files likely affected | Verification command | Presentation value |
| --- | --- | --- | --- | --- | --- |
| Extend the transactional outbox with broader idempotency coverage and retry or DLQ handling for messaging | S30 now gives the producer services a real outbox, but endpoint coverage, DLQ strategy, and richer recovery policies are still limited. | Medium | `backend/enrollment-service/*`; `backend/billing-service/*`; `backend/notification/*`; `db/*` | `rg -n "outbox|idempot|DLQ|retry" backend db` | Medium: valuable for roadmap discussion after the current delivery, not required now. |
